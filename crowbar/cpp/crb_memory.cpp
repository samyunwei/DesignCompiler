...
static void error_handler(MEM_Controller controller, char *filename, int line, char *msg) {
    ...
    if (controller->failMode == MEM_FAIL_AND_EXIT) {
        exit(1);
    }
}
...
static void rechian_block(MEM_Controller controller, Header *header) {
    if (header->s.prev) {
...
static void unchian_block(MEM_Controller controller, Header *header) {
    if (header->s.prev) {
...
void *MEM_malloc_func(MEM_Controller controller, char *filename, int line, size_t size) {
    ...
    set_header((Header_tag *) ptr, size, filename, line);
...
void *MEM_realloc_func(MEM_Controller controller, char *filename, int line, void *ptr, size_t size) {
    ...
    if (ptr) {
        ...
        *((Header *) new_ptr) = old_header;
        ...
        rechian_block(controller, (Header *) new_ptr);
        set_tail(new_ptr, alloc_size);
    } else {
        set_header((Header *) new_ptr, size, filename, line);
        set_tail(new_ptr, alloc_size);
